...
import { Settings, User, Database, Download, Trash2, AlertTriangle, CheckCircle, XCircle } from 'lucide-react';
...
    const confirmacion = window.prompt('Esta acción eliminará TODOS los datos almacenados .\n\nEscribe "ELIMINAR" para confirmar:');
    if (confirmacion === 'ELIMINAR') {
       await productService.clearAll();
       await   clientService.clearAll();
      await saleService.clearAll();
...
          </button>
        </li>
        {/* Modificación aquí: La pestaña 'Datos' solo se muestra si el rol es 'admin' */}
        {user?.role === 'admin' && (
          <li className="nav-item">
...
      {/* Contenido de la pestaña Datos */}
      {/* Modificación aquí: El contenido de la pestaña 'Datos' solo se renderiza si el rol es 'admin' */}
      {pestanaActiva === 'data' && user?.role === 'admin' && (
        <div className="row">
          <div className="col-md-6">
            <div className="card">
...
                    <small className="text-muted d-block mt-2">Esta acción eliminará permanentemente todos los datos de su navegador (productos, clientes, ventas y usuarios) que se hayan guardado.</small>
                  </div>
                )}
              </div>
            </div>
...
                      <dt className="col-sm-5">Almacenamiento:</dt><dd className="col-sm-7">db.json</dd>
                    </dl>
                  </div>
                  <div className="col-md-6">
                    <dl className="row">
...
                  <button className="btn btn-outline-primary" onClick={() => window.open('https://github.com/Neison1301/PROYECTO-JSA', '_blank')}>Documentación</button>
                  <button className="btn btn-outline-secondary" onClick={() => alert('Soporte técnico: neisonssj2@gmail.com')}>Soporte Técnico</button>
                  <button className="btn btn-outline-info" onClick={() => alert('Proyecto-JSA\nDesarrollado con React + Vite\n© 2025')}>Acerca de</button>
                </div>
              </div>
            </div>
          </div>
