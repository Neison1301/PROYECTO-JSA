...
const BarraLateral: React.FC<PropiedadesBarraLateral> = ({
  estaColapsada,
  alAlternar,
}) => {
  ...
  return (
    <div className={`sidebar ${estaColapsada ? "sidebar-collapsed" : ""}`}>
      <div className="sidebar-header">
        <button className="btn btn-lin text-white p-0" onClick={alAlternar}>
          <Menu size={24} />
        </button>
        {!estaColapsada && (
          <div className="mt-2">
          ...
          .map((item) => (
            <div key={item.componente} className="nav-item">
              ...
              >
                <item.icono size={20} />
                {!estaColapsada && (
                  <span className="ms-3">{item.etiqueta}</span>
                )}
              </button>
            </div>
          ))}
